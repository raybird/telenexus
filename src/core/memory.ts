...
export class MemoryManager {
  ...
  private initTable() {
    ...
    this.db
      ...
      .run();
    // memory-health 的 24h 統計以 role 過濾但不帶 user_id,前兩個索引都是 user_id 開頭而幫不上忙,
    // 使其退化成整表掃描(實測 115K 列 / 212MB 時單次 40ms,佔整份 context snapshot 成本的三分之二)。
    this.db
      .prepare(
    ...
    // runtime_issues：跨重啟保留錯誤事件，給 error-summary / 告警面板查詢
    this.db
      .prepare(
      ...
      .run();
    this.db.prepare(`CREATE INDEX IF NOT EXISTS idx_issues_ts ON runtime_issues(ts)`).run();
    this.db
      .prepare(`CREATE INDEX IF NOT EXISTS idx_issues_scope_ts ON runtime_issues(scope, ts)`)
      .run();
  }

  appendRuntimeIssue(scope: string, message: string, timestamp: number): void {
    // 分隔符寫成跳脫序列而非直接嵌入 NUL 位元組:兩者在執行期是同一個字元(U+0000),
    // 但真正的 NUL 會讓 git 把整個檔案判為二進位而顯示不出 diff。
    const fingerprint = `${scope}\u0000${message.slice(0, 120)}`;
    const dedupeSince = timestamp - 60_000;
    const existing = this.db
      .prepare(
        `SELECT id FROM runtime_issues WHERE fingerprint = ? AND ts >= ? ORDER BY ts DESC LIMIT 1`
      )
  ...
  countIssuesByScope(sinceMs: number): Array<{ scope: string; count: number }> {
    return this.db
      .prepare(
        `SELECT scope, COUNT(*) as count FROM runtime_issues WHERE ts >= ? GROUP BY scope ORDER BY count DESC`
      )
  ...
  recentRuntimeIssues(
    ...
    return this.db
      .prepare(
        `SELECT ts, scope, message FROM runtime_issues WHERE ts >= ? ORDER BY ts DESC LIMIT ?`
      )
      .all(sinceMs, limit) as Array<{ ts: number; scope: string; message: string }>;
  }

  purgeOldRuntimeIssues(beforeMs: number): number {
    const info = this.db.prepare(`DELETE FROM runtime_issues WHERE ts < ?`).run(beforeMs);
  ...
  getAllCanonicalSummaries(userId: string): SummaryMessage[] {
    const stmt = this.db.prepare(`
      SELECT id, role, content, summary, impact_level, tags, timestamp
      FROM messages
      WHERE user_id = ?
    ...
    return rows.map((row) => this.toSummaryMessage(row));
  }
